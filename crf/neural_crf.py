...
class ConditionalRandomField(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # 表示label当中前一个tag和后一个tag的变换关系
        self.transitions = torch.nn.Parameter(torch.Tensor(num_tags, num_tags))

        # _constraint_mask indicates valid transitions (based on supplied constraints).
        # Include special start of sequence (num_tags + 1) and end of sequence tags (num_tags + 2)i
        # 有时候我们可能不允许某些tag之间能够前后转换，这里我们加上一个constrain mask
        ...
        # 给sequence的开头和结尾两个特殊的transition logits
    ...
    def _input_likelihood(self, logits: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        计算所有state sequences的 logsumexp
        logits: 其实是我们的emission probability，也就是从x特征来计算每个位置tag的分数
        """
        batch_size, sequence_length, num_tags = logits.size()

        # Transpose batch size and sequence dimensions
        mask = mask.float().transpose(0, 1).contiguous() # sequence_length, batch_size, num_tags
        logits = logits.transpose(0, 1).contiguous() # sequence_length, batch_size, num_tags

        # Initial alpha is the (batch_size, num_tags) tensor of likelihoods combining the
        # transitions to the initial states and the logits for the first timestep.
        # 这里计算alpha的初始值，也就是第一步每个tag的state分数
        if self.include_start_end_transitions:
        ...
        # 这里我们计算从t-1位置的tag转变到t位置的tag所得的分数。每个state都会有num_tags个分数，
        # 那么到了下一个state我们会有 (num_tags, num_tags)个分数，因为每个tag都有可能转变到任意一个tag
        # 考虑到batch_size，我们最终会得到一个 (batch_size, num_tags, num_tags) 的变换矩阵分数
        for i in range(1, sequence_length):
            ...
            # logsumexp: 如名字所述，其实就是先对每个位置算exp，然后全部相加，最后得到的和求log.
            # tensor.exp().sum(dim, keep=keepdim).log()
            # 回顾linear chain CRF的公式，分子是exp(xxx)，分母是exp(xxx)的求和。这里我们给分母取上一个log，为了让计算更加稳定。
            alpha = (util.logsumexp(inner, 1) * mask[i].view(batch_size, 1) +
                     alpha * (1 - mask[i]).view(batch_size, 1))

        # Every sequence needs to end with a transition to the stop_tag.
        # 最后一个位置，再加上一个结尾的transition分数
        if self.include_start_end_transitions:
    ...
    def _joint_likelihood(self,
        ...
        # Transpose batch size and sequence dimensions:
        logits = logits.transpose(0, 1).contiguous() # sequence_length, batch_size, _
        mask = mask.float().transpose(0, 1).contiguous() # sequence_length, batch_size, _
        tags = tags.transpose(0, 1).contiguous() # sequence_length, batch_size
        ...
        for i in range(sequence_length - 1):
            ...
            current_tag, next_tag = tags[i], tags[i+1] # 当前的tag和下一个tag

            # The scores for transitioning from current_tag to next_tag
            # 从当前tag转换到下一个tag的转换分数
            transition_score = self.transitions[current_tag.view(-1), next_tag.view(-1)] 

            # The score for using current_tag
            # 从当前tag生成当前位置feature(x)的分数
    ...
    def viterbi_tags(self,
        ...
        使用viterbi算法找到给定输入sequence最可能的tag sequence. 
        """
